...
export function EarningsScreen() {
  ...
  return (
    <div className={base}>
      ...
      <p className="text-[27px] font-extrabold leading-none mb-1">{formatTenge(scenario.earned)}</p>
      ...
      <div className="flex items-end gap-[3px] h-12 mb-4">
        {weeklyEarnings.map((v, i) => (
          <div key={i} className="w-[7px] rounded-full bg-orange/70" style={{ height: `${(v / max) * 100}%` }} />
        ))}
      </div>
      <div className="space-y-2 mt-auto">
        <div className="flex justify-between text-[11px]">
          <span className="text-ink-soft">Already received</span>
          <span className="font-semibold">{formatTenge(scenario.alreadyReceived)}</span>
        </div>
        <div className="flex justify-between text-[11px] pt-2 border-t border-ink/[0.07]">
          <span className="text-ink-soft">Available</span>
          <span className="font-semibold">{formatTenge(scenario.available)}</span>
        </div>
      </div>
    </div>
  );
...
export function ExpensesScreen() {
  return (
    <div className={base}>
      ...
      <p className="text-[27px] font-extrabold leading-none mb-4">{formatTenge(scenario.upcomingExpensesTotal)}</p>
      ...
      <div className="mt-auto rounded-xl bg-cream-orange p-3">
        <p className="text-[10.5px] text-orange-dark font-medium leading-snug">
...
export function RecommendationScreen() {
  return (
    <div className={base}>
      ...
      <p className="text-[29px] font-extrabold leading-none mb-4">{formatTenge(scenario.recommended)}</p>
      ...
      <span className="inline-block w-fit text-[10px] font-semibold text-green bg-green/10 px-2.5 py-1 rounded-full mb-4">
        Comfortable
      </span>
      <div className="mt-auto flex items-center gap-1 text-[11.5px] font-semibold text-orange-dark">
...
export function WhatIfScreen() {
  ...
  return (
    <div className={base}>
      ...
      <p className="text-[23px] font-extrabold leading-none mb-2">{formatTenge(result.amount)}</p>
      <span className="inline-block w-fit text-[10px] font-semibold text-orange-dark bg-orange/10 px-2.5 py-1 rounded-full mb-3">
        {result.label}
      </span>
  ...
  return (
    <div className={base}>
      <div className="flex items-center gap-1.5 mb-3">
        <MessageCircle size={13} className="text-orange-dark" />
      ...
      <div className="space-y-2.5 text-[10.5px] leading-snug">
        ...
        <div className="mr-auto max-w-[90%] bg-cream-soft rounded-xl rounded-tl-sm px-3 py-2">
