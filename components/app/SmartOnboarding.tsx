...
const questions = [
  ...
  { key: "financialPressure", type: "choice", title: "What usually puts the most pressure on your budget before payday?", text: "Choose what feels most relevant.", options: ["Unexpected expenses", "Regular bills", "Food & everyday spending", "Transport", "Large planned purchases", "Nothing in particular"] },
  ...
  async function finish() {
    ...
    try {
      await api.onboarding.save({
        pay_frequency: answers.payFrequency,
        monthly_income_range: answers.monthlyIncomeRange,
        payday_range: answers.paydayRange,
        financial_pressure: answers.financialPressure,
        early_access_use_case: answers.earlyAccessUseCase,
        ai_help_preference: answers.aiHelpPreference,
      });
      ...
      <div className="grid items-center gap-12 py-14 lg:grid-cols-[0.72fr_1.28fr] lg:py-20">
        ...
        <section className="rounded-[28px] border border-ink/[0.09] bg-white/75 p-6 shadow-[0_28px_70px_-48px_rgba(21,21,21,0.5)] sm:p-10">
          <AnimatePresence mode="wait"><motion.div key={step} initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }} transition={{ duration: 0.28 }}>
            ...
            {question.type === "text" ? <label className="mt-9 block text-[13px] font-semibold">{question.label}<input autoFocus value={String(currentValue)} onChange={(event) => updateAnswer(event.target.value)} placeholder={question.placeholder} className="mt-2 w-full rounded-[12px] border border-ink/[0.12] bg-cream px-4 py-4 text-[16px] outline-none transition-colors focus:border-orange" /></label> : <div className="mt-9 grid gap-3">{question.options?.map((option) => <button key={option} type="button" onClick={() => updateAnswer(option)} className={`flex w-full items-center justify-between rounded-[14px] border px-4 py-4 text-left text-[14px] font-semibold transition-all ${currentValue === option ? "border-orange bg-cream-orange text-ink shadow-[0_10px_24px_-20px_rgba(201,71,24,0.8)]" : "border-ink/[0.1] bg-cream/60 text-ink-soft hover:border-orange/50 hover:text-ink"}`}>{option}{currentValue === option && <Check size={17} className="text-orange-dark" />}</button>)}</div>}
