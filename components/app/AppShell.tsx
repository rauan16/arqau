...
const employeeNav = [
  ...
  { label: "My earnings", href: "/dashboard/finance", icon: BarChart3 },
  { label: "Withdraw", href: "/dashboard/get-money", icon: CircleDollarSign },
  { label: "AI assistant", href: "/dashboard/ai", icon: MessageCircle },
  { label: "Expenses", href: "/dashboard/history", icon: History },
  { label: "Profile", href: "/dashboard/settings", icon: Settings },
];

const employerNav = [
  { label: "Overview", href: "/business", icon: Home },
  { label: "Employees", href: "/business/employees", icon: Users },
  { label: "Payroll", href: "/business/payroll", icon: BriefcaseBusiness },
  { label: "EWA", href: "/business/requests", icon: CircleDollarSign },
  { label: "Financial Wellbeing", href: "/business/analytics", icon: ChartNoAxesCombined },
    ...
    <div className="min-h-screen bg-cream text-ink lg:grid lg:grid-cols-[248px_1fr]">
      <aside className={`${open ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-50 w-[248px] border-r border-ink/[0.08] bg-cream-soft px-5 py-6 transition-transform lg:static lg:translate-x-0`}>
        ...
        <nav className="space-y-1">
          {nav.map(({ label, href, icon: Icon }) => {
            ...
            return <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-[10px] px-3 py-3 text-[14px] font-semibold transition-colors ${active ? "bg-green-deep text-cream" : "text-ink-soft hover:bg-green-soft hover:text-green-deep"}`}><Icon size={17} strokeWidth={1.8} />{label}</Link>;
          })}
        </nav>
        <div className="mt-12 border-t border-ink/[0.08] pt-5">
          {employer ? (
            <Link href="/business/register" className="flex items-center justify-between rounded-[14px] border border-green/25 bg-green-soft p-3 text-[13px] font-semibold text-green-deep">Update business <ChevronRight size={16} /></Link>
          ) : (
            <Link href="/business/register" className="flex items-center justify-between rounded-[14px] border border-green/25 bg-green-soft p-3 text-[13px] font-semibold text-green-deep">Register business <ChevronRight size={16} /></Link>
          )}
        </div>
        <div className="absolute bottom-6 left-5 right-5 flex items-center gap-3 border-t border-ink/[0.08] pt-5"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-green-deep text-[13px] font-bold text-cream">{initial}</div><div><p className="text-[13px] font-bold">{displayName}</p><p className="text-[11px] text-ink-soft">{user?.email || "User account"}</p></div></div>
      </aside>
      ...
      <main className="min-w-0">
        <header className="sticky top-0 z-30 flex h-[76px] items-center justify-between border-b border-ink/[0.08] bg-cream/90 px-5 backdrop-blur-md sm:px-8 lg:px-12">
          ...
          <div className="ml-auto flex items-center gap-3"><span className="hidden text-[12px] text-ink-soft md:block">{employer ? "Business account" : "Personal account"}</span><Link href={employer ? "/business/settings" : "/dashboard/settings"} className="flex h-9 w-9 items-center justify-center rounded-full bg-green-deep text-[13px] font-bold text-cream focus-ring">{initial}</Link></div>
        </header>
