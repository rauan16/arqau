...
function Metric({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) {
  return <div className={panel}>
    <div className="flex items-center justify-between text-orange-dark">
      <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-ink-soft">{label}</p>
      {icon}
    </div>
    <p className="mt-5 text-[28px] font-extrabold tracking-tight">{value}</p>
  </div>;
}
        ...
        {showAdd && (
          <div className="mt-4 space-y-3">
            <div className="flex gap-2">
              <button onClick={() => setTxType("income")} className={`flex-1 rounded-[10px] border px-3 py-2 text-[13px] font-semibold transition-colors ${txType === "income" ? "border-green bg-green/10 text-green" : "border-ink/10 hover:border-green/30"}`}>Income</button>
              <button onClick={() => setTxType("expense")} className={`flex-1 rounded-[10px] border px-3 py-2 text-[13px] font-semibold transition-colors ${txType === "expense" ? "border-orange bg-orange/10 text-orange-dark" : "border-ink/10 hover:border-orange/30"}`}>Expense</button>
            </div>
            <input type="number" value={txAmount} onChange={(e) => setTxAmount(e.target.value)} placeholder="Amount (₸)" className="w-full rounded-[10px] border border-ink/[0.12] bg-cream px-4 py-3 text-[14px] outline-none transition-colors focus:border-orange" />
            <input value={txCategory} onChange={(e) => setTxCategory(e.target.value)} placeholder="Category (optional)" className="w-full rounded-[10px] border border-ink/[0.12] bg-cream px-4 py-3 text-[14px] outline-none transition-colors focus:border-orange" />
            <input value={txMerchant} onChange={(e) => setTxMerchant(e.target.value)} placeholder="Description (optional)" className="w-full rounded-[10px] border border-ink/[0.12] bg-cream px-4 py-3 text-[14px] outline-none transition-colors focus:border-orange" />
          ...
          <div className="mt-6 space-y-3">
            {transactions.slice(0, 20).map((item) => (
              <div key={item.id} className="flex items-center justify-between border-b border-ink/[0.06] pb-3 text-[13px] last:border-0">
                ...
                <span className={`font-bold ${item.type === "income" ? "text-green" : "text-orange-dark"}`}>
...
export function AIPage() {
  ...
  async function handleAnalyze() {
    ...
    if (!amount || amount <= 0) {
      setAnalysisError("Please enter a valid amount");
      return;
    }
  ...
  return <PageFrame eyebrow="ARQAU assistant" title="Explore your financial context." copy="Understand your earned income, expenses and withdrawal options with ARQAU analysis.">
    <div className="grid gap-5 lg:grid-cols-[1fr_0.35fr]">
      <section className={`${panel} max-w-[760px]`}>
        ...
        {earned && (
          <div className="mb-4 rounded-[12px] bg-cream px-4 py-3 text-[13px] text-ink-soft">
            Available to access: <span className="font-bold text-orange-dark">{formatTenge(earned.available_amount_minor)} ₸</span>
          </div>
        )}
        <div className="flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-[10px] border border-ink/[0.1] bg-cream px-4 py-3 text-[14px] outline-none transition-colors focus:border-orange"
          />
        ...
        {analysis && (
          <div className="mt-4 space-y-3 rounded-[12px] border border-ink/[0.08] bg-cream p-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-[13px] font-bold">{formatTenge(analysis.remaining_amount)} ₸</span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <p className="mt-2 border-t border-ink/[0.08] pt-2 text-[13px] leading-relaxed text-ink-soft">{analysis.explanation}</p>
            <p className="text-[11px] text-ink-soft italic">{analysis.disclaimer}</p>
          </div>
        )}
      </section>
      <aside className="rounded-[20px] border border-orange/20 bg-cream-orange p-5">
        <div className="flex items-center gap-2 text-orange-dark"><Sparkles size={18} /><h2 className="font-bold">Suggested questions</h2></div>
        <div className="mt-5 space-y-3">
          {["What can I consider before payday?", "How could a reserve help?", "Should I access earned wages?"].map((prompt) => (
            <button key={prompt} onClick={() => { setMessage(prompt); }} className="w-full rounded-[10px] border border-orange/20 bg-cream px-3 py-2 text-left text-[13px] font-semibold text-orange-dark transition-colors hover:bg-orange hover:text-cream">
              {prompt}
            </button>
          ))}
        </div>
    ...
    <section className={`${panel} mt-5 max-w-[760px]`}>
      ...
      <div className="max-h-[300px] space-y-3 overflow-y-auto">
        ...
        {chatHistory.map((item, i) => (
          <div key={i} className={`max-w-[82%] rounded-[16px] px-4 py-3 text-[14px] leading-relaxed ${item.from === "you" ? "ml-auto bg-ink text-cream" : "bg-cream-orange text-ink"}`}>
            {item.text}
          </div>
        ))}
      </div>
...
export function GetMoneyPage() {
  ...
  if (loading) {
    return <PageFrame eyebrow="Get money" title="Make the next decision with context." copy="Loading...">
      <div className="flex h-40 items-center justify-center"><Loader2 className="animate-spin text-orange" size={24} /></div>
    </PageFrame>;
  }
  ...
  return <PageFrame eyebrow="Get money" title="Make the next decision with context." copy="Access earned wages with clear context. Backend calculates your available amount.">
    {error && <div className="mb-4 rounded-[10px] border border-red-200 bg-red-50 px-4 py-3 text-[13px] text-red-700 flex items-center gap-2"><AlertCircle size={16} />{error}</div>}
    {success && <div className="mb-4 rounded-[10px] border border-green/20 bg-green/10 px-4 py-3 text-[13px] text-green flex items-center gap-2"><Check size={16} />{success}</div>}
      ...
      <section className={panel}>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <p className="mt-1 text-[24px] font-extrabold">{formatTenge(earned.earned_amount_minor)} ₸</p>
          </div>
          <div>
            <p className="text-[12px] text-ink-soft">Accessed</p>
            <p className="mt-1 text-[24px] font-extrabold">{formatTenge(earned.accessed_amount_minor)} ₸</p>
          </div>
          <div>
            <p className="text-[12px] text-ink-soft">Available</p>
            <p className="mt-1 text-[24px] font-extrabold text-orange-dark">{formatTenge(available)} ₸</p>
          </div>
        </div>

        <div className="mt-6 border-t border-ink/[0.08] pt-4">
          <h3 className="mb-1 text-[15px] font-bold">Request access</h3>
          <p className="mb-3 text-[13px] text-ink-soft">Available after request: <span className="font-bold text-ink">{formatTenge(Math.max(0, available - Number(amount || 0)))} ₸</span></p>
          <div className="space-y-3">
            <input type="number" value={amount} onChange={(e) => { setAmount(e.target.value); setError(null); }} placeholder={`Amount up to ${formatTenge(available)} ₸`} className="w-full rounded-[10px] border border-ink/[0.12] bg-cream px-4 py-3 text-[14px] outline-none transition-colors focus:border-orange" />
            <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Reason (optional)" className="w-full rounded-[10px] border border-ink/[0.12] bg-cream px-4 py-3 text-[14px] outline-none transition-colors focus:border-orange" />
      ...
      {allItems.length === 0 ? (
        ...
        <>
          <div className="relative max-w-[360px]">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search activity" aria-label="Search activity" className="w-full rounded-[10px] border border-ink/[0.1] bg-cream py-3 pl-9 pr-3 text-[14px] outline-none transition-colors focus:border-orange" />
          </div>
              ...
              <tbody>
                {filtered.map((item) => (
                  <tr key={item.id} className="border-b border-ink/[0.06] last:border-0">
                    ...
                    <td className={`py-4 text-right font-bold ${item.type === "income" ? "text-green" : "text-orange-dark"}`}>
    ...
    <div className="grid max-w-[760px] gap-5">
      ...
      <section className={panel}>
        ...
        <button
          ...
          className="mt-4 rounded-[10px] border border-orange/30 bg-cream-orange px-4 py-2 text-[13px] font-bold text-orange-dark transition-colors hover:bg-orange hover:text-cream"
        >
          Sign out
        </button>
      </section>
